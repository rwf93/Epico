...
VkBufferCreateInfo buffer_create_info(VkDeviceSize size, VkBufferUsageFlags usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VkBufferCreateFlags flags = 0);
VmaAllocationCreateInfo allocation_create_info(VmaAllocationCreateFlags flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT, VkMemoryPropertyFlags preferred_flags = 0, VmaMemoryUsage usage = VMA_MEMORY_USAGE_AUTO, float priority = 1.0f);
